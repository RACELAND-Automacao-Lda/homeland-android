...
import io.homeland.companion.android.common.util.sensorCoreSyncChannel
...
abstract class SensorReceiverBase : BroadcastReceiver() {
    companion object {
        const val ACTION_UPDATE_SENSOR = "io.homeland.companion.android.UPDATE_SENSOR"
                ...
                if (setting.value != "" && intent.action == setting.value) {
                    ...
                    ioScope.launch {
                        try {
                            ...
                            Log.d(tag, "Event successfully sent to HomeLand")
                        } catch (e: Exception) {
                            Log.e(
                                tag,
                                "Unable to send event data to HomeLand",
                                e
                            )
                        }
                    }
    ...
    suspend fun updateSensors(
        ...
        val currentHAversion = integrationUseCase.getHomeLandVersion()
        val supportsDisabledSensors = integrationUseCase.isHomeLandVersionAtLeast(2022, 6, 0)
