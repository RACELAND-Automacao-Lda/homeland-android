...
import io.homeland.companion.android.onboarding.manual_setup.ManualSetupActivity
import io.homeland.companion.android.util.LoadingView
...
class OnboardingActivity : AppCompatActivity(), OnboardingView {
    ...
    private fun startPhoneSignIn(instance: HomeLandInstance?) {
        lifecycleScope.launch {
            showLoading()
            try {
                val url = "homeland://wear-phone-signin${if (instance != null) "?url=${instance.url}" else ""}"
    ...
    override fun onInstanceFound(instance: HomeLandInstance) {
    ...
    override fun onInstanceLost(instance: HomeLandInstance) {
        if (adapter.servers.contains(instance)) {
    ...
    private fun findExistingInstances() {
        ...
        Tasks.await(Wearable.getDataClient(this).getDataItems(Uri.parse("wear://*/home_land_instance"))).apply {
    ...
    private fun requestInstances() {
        ...
        capabilityInfo.nodes.forEach { node ->
            Wearable.getMessageClient(this).sendMessage(
                ...
                addOnSuccessListener { Log.d(TAG, "requestInstances: request homeland instances from $node.id: ${node.displayName}") }
                addOnFailureListener { Log.w(TAG, "requestInstances: failed to request homeland instances from $node.id: ${node.displayName}") }
            }
        }
    }
