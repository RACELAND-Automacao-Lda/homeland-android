...
class OnboardingPresenterImpl @Inject constructor(
        ...
        dataEvents.forEach { event ->
            if (event.type == DataEvent.TYPE_CHANGED) {
                event.dataItem.also { item ->
                    if (item.uri.path?.compareTo("/home_land_instance") == 0) {
                        Log.d(TAG, "onDataChanged: found home_land_instance")
    ...
    override fun getInstance(map: DataMap): HomeLandInstance {
        map.apply {
            return HomeLandInstance(
                getString("name", ""),
                URL(getString("url", "")),
                getString("version", "")
            )
